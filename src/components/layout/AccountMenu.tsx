...
export function AccountMenu() {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen((o) => !o)} className="flex items-center" aria-label="Account">
        {data.avatar_url ? (
          <img
            src={data.avatar_url}
            alt=""
      ...
      {open && (
        <>
          ...
          <div className="absolute top-9 right-0 z-50 w-56 rounded-2xl border border-white/10 bg-ink-850/95 p-3 shadow-xl backdrop-blur-xl">
            <p className="truncate font-display text-sm font-semibold">{data.name || data.email}</p>
            {data.email && data.name && (
              <p className="truncate text-xs text-white/40">{data.email}</p>
            )}
            <button
              onClick={() => {
                clearToken();
              ...
              className="mt-3 w-full rounded-full border border-white/10 py-2 text-xs font-semibold text-white/70 transition-colors hover:text-white"
            >
              {t("auth.signOut")}
            </button>
          </div>
