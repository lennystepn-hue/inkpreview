  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      >
        {design.clean_png_url && (
          <img
            ...
            alt={design.prompt}
            className="mx-auto max-h-[44dvh] w-full object-contain p-5"
          />
        )}
      </motion.div>

      <p className="text-center text-sm text-white/50">„{design.prompt}"</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button size="lg" onClick={onTryOn}>
          {t("result.tryOn")}
        </Button>
        ...
        <div className="grid grid-cols-3 gap-2">
          {all.map((v) => (
            <button
              ...
              onClick={() => onChoose(v)}
              className={cn(
                "aspect-square overflow-hidden rounded-2xl border bg-white transition-colors",
                v.id === design.id ? "border-acid" : "border-white/10 hover:border-white/30",
              )}
            >
      ...
      <button onClick={onReset} className="text-center text-xs text-white/40 hover:text-white/70">
        {t("result.newDesign")}
      </button>
    </div>
  );
